...
class AddPatient(View):
    def get(self, request):
        ...
        return render(request, 'form.html', {'customers': customers})
    ...
    def add_patient(self, request):
        ...
        if Users.objects.filter(patient_id=patient_id).exists():
            ...
            data = {"error": error_message, "customers": customers}
        ...
        error_message = self.validate_customer(patient_name, age, phone, email, address, patient_id)
        if error_message:
            customers = Users.objects.order_by('-id')  # Reverse order by id
            data = {"error": error_message, "customers": customers, "values": value}
            return render(request, 'form.html', data)
    ...
    def validate_customer(self, patient_name, age, phone, email, address, patient_id):
        if not patient_id:
    ...
    def save_modalities(self, request):
        ...
        for key in request.POST.keys():
            if any(key.startswith(modality) for modality in ['xray_', 'ecg_', 'pft_', 'audiometry_', 'optometry_', 'vitals_', 'sample_collection_']):
                ...
                # Update the modalities based on the form data
                patient.xray = 'xray_{}'.format(patient_id_from_checkbox) in request.POST
                patient.ecg = 'ecg_{}'.format(patient_id_from_checkbox) in request.POST
                patient.pft = 'pft_{}'.format(patient_id_from_checkbox) in request.POST
                patient.audiometry = 'audiometry_{}'.format(patient_id_from_checkbox) in request.POST
                patient.optometry = 'optometry_{}'.format(patient_id_from_checkbox) in request.POST
                patient.vitals = 'vitals_{}'.format(patient_id_from_checkbox) in request.POST
                patient.sample_collection = 'sample_collection_{}'.format(patient_id_from_checkbox) in request.POST
